...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, **stats_kwargs):
    ...
    if opts.cache:
        ...
        print("cache file in dataset metrics computing: ", cache_file)
    ...
    for images, _labels in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        # print(f"dataset: {dataset}, images: {images.shape}")
        if images.shape[1] == 1:
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, jit=False, **stats_kwargs):
    ...
    def run_generator(z, c, i):
        img = G(z=z, c=c, **opts.G_kwargs)
        img = (img * 127.5 + 128).clamp(0, 255).to(torch.uint8)
        # print("img shape: ", img.shape)
        # Image.fromarray(img[0].permute(1,2,0).cpu().numpy(), 'RGB').save(f'./tmp/i_{i:03d}.png')
        return img
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            c =[rand_light() for _i in range(batch_gen)]
            # print("c: len, ", len(c))
            # cond_li = torch.from_numpy(rand_light()).unsqueeze(0).to(device)

            c = torch.from_numpy(np.stack(c)).pin_memory().to(opts.device)
            images.append(run_generator(z, c, _i))
