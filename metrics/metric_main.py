...
def report_metric(result_dict, run_dir=None, snapshot_pkl=None):
    ...
    key = snapshot_pkl.split('-')[-1]
    key = key.split('.pkl')[0]
    print(key, fid)

    with open(os.path.join(run_dir, 'fid.txt'), 'a') as f:
        f.write(f'{key}:{fid}' + "\n")  
...
def fid50k_full(opts):
    ...
    fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=50000) #50000
